...
function addBot(text,suggestions,resources,journey,search,viewing,next,lead,handoff){
 ...
 if(handoff&&lead){const landlord=lead.persona==='landlord';const box=document.createElement('div');box.className='ai-handoff-summary'+(landlord?' landlord-result':'');box.innerHTML='<small>'+(landlord?'YOUR PROPERTY RENTAL PLAN':'READY FOR WANG')+'</small><b>'+esc(landlord?(lead.property||'Property ready for review'):(lead.need||'Your rental request'))+'</b>'+(landlord?'<div class="landlord-facts">'+[['Property type',lead.property_type],['Bedrooms',lead.bedrooms],['Size',lead.size],['Available',lead.availability||lead.target_date],['Furnishing',lead.furnishing],['Expected rent',lead.expected_rent],['Marketing',lead.marketing_status]].filter(x=>x[1]).map(x=>'<span><em>'+esc(x[0])+'</em>'+esc(x[1])+'</span>').join('')+'</div>':'')+'<p>'+esc(lead.help_needed||'Wang can continue from here without you repeating everything.')+'</p>';d.appendChild(box)}
 if(handoff){const a=document.createElement('a');a.className='mini ai-wa';a.target='_blank';a.rel='noopener';a.href=wa(latestLead&&latestLead.persona==='landlord'?'I would like Wang to review my property and rental strategy.':'');a.textContent=latestLead&&latestLead.persona==='landlord'?'GET MY RENTAL ASSESSMENT →':'CONTINUE WITH WANG ON WHATSAPP →';d.appendChild(a)}
